...
export function SearchRunsPage() {
  ...
  const state = useLoad(() => api<SearchRun[]>(`/search-runs?${params}`), [provider, mode, status]);
  return <><PageHeader eyebrow="Trace" title="Search runs"><span className="count">{state.data?.length ?? 0} runs</span></PageHeader>
    <Panel><div className="filters run-filters"><select aria-label="Provider" value={provider} onChange={event=>setProvider(event.target.value)}><option value="">All providers</option>{["scopus","arxiv","acl_anthology","ieee_xplore","web_of_science","acm_dl"].map(value=><option key={value}>{value}</option>)}</select><select aria-label="Mode" value={mode} onChange={event=>setMode(event.target.value)}><option value="">Explore and Save</option><option value="explore">explore</option><option value="save">save</option></select><select aria-label="Run status" value={status} onChange={event=>setStatus(event.target.value)}><option value="">All statuses</option>{["complete","partial","failed"].map(value=><option key={value}>{value}</option>)}</select></div>
      <Feedback loading={state.loading} error={state.error}>{state.data?.length ? <div className="run-table" role="table">{state.data.map(run=><article key={run.search_run_id}><div><Link to={`/search-runs/${run.search_run_id}`}><strong>{run.search_code}</strong></Link><small>{run.executed_at_utc}</small></div><div><span>{run.provider}</span><small>{run.mode} · {run.retrieved_count} retrieved</small></div><Status tone={run.status==="complete"?"good":run.status==="partial"?"warn":"bad"}>{run.status}</Status></article>)}</div> : <Empty>No search runs match these filters.</Empty>}</Feedback>
    </Panel></>;
}
...
export function SearchRunDetailPage() {
  ...
  return <><PageHeader eyebrow="Search run" title={run?.search_code || "Search run detail"}>{run && <Status tone={run.status==="complete"?"good":run.status==="partial"?"warn":"bad"}>{run.status}</Status>}</PageHeader>
